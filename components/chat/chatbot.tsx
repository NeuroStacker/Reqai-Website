...
export function Chatbot({ 
  ...
  const exportBRD = () => {
    ...
    const cleanContent = lastResponse.replace(/\*\*/g, '');
    
    // Create Word document
    const doc = new Document({
      sections: [{
        properties: {},
        children: generateBRDSections(cleanContent),
      }],
    });
  ...
  // Function to parse BRD content and create formatted sections
  const generateBRDSections = (content: string) => {
    ...
    // Create a map to collect content for each section
    const sectionContent: Record<string, string[]> = {};
    standardSections.forEach(section => {
      sectionContent[section] = [];
    });
    
    // Split content by lines and categorize into sections
    ...
    lines.forEach(line => {
      ...
      const isSectionHeader = /^#+\s+(.+)/.test(line) || 
        ...
        line.includes('Stakeholders:') || 
        line.includes('Functional Requirements:') || 
        line.includes('Non-Functional Requirements:') ||
        line.includes('Definitions:') ||
        line.includes('Business Rules:') ||
      ...
      if (isSectionHeader) {
        ...
        let sectionName = line.replace(/^#+\s+/, '').replace(/:$/, '');
        
        // Find matching standard section
        currentSection = standardSections.find(std => 
          sectionName.includes(std) || std.includes(sectionName)
        ) || sectionName;
        
        // Initialize section content array if it doesn't exist
    ...
    // Create tables for sections in standard order
    let sectionCount = 1;
    standardSections.forEach(section => {
      if (sectionContent[section] && sectionContent[section].length > 0) {
        // Create section with number prefix
        const numberedSection = `${sectionCount}. ${section}`;
        paragraphs.push(createSectionTable(numberedSection, sectionContent[section].join('\n')));
        paragraphs.push(new Paragraph("")); // Add spacing
        sectionCount++;
      }
    });
    
    // Add any other sections that weren't in our standard list
    Object.keys(sectionContent).forEach(section => {
      if (!standardSections.includes(section) && sectionContent[section].length > 0) {
        const numberedSection = `${sectionCount}. ${section}`;
        paragraphs.push(createSectionTable(numberedSection, sectionContent[section].join('\n')));
        paragraphs.push(new Paragraph("")); // Add spacing
        sectionCount++;
  ...
  // Function to create a section table with header and content
  const createSectionTable = (sectionName: string, content: string) => {
    return new Table({
      width: {
        size: 100,
      ...
      rows: [
        ...
        new TableRow({
          children: [
            ...
            // Section details in right column
            new TableCell({
              width: {
                size: 75,
                type: WidthType.PERCENTAGE,
  ...
  // Function to process details content - detect and format numbered lists
  const processDetailsContent = (content: string): Paragraph[] => {
    const lines = content.split('\n');
    ...
    if (isNumberedList) {
      ...
      lines.forEach(line => {
        ...
        if (match) {
          ...
          // Non-empty line that's not numbered
          paragraphs.push(
            new Paragraph({
              text: line.trim()
            })
